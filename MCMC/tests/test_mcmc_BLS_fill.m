...
scale = max(S); 
E = S/scale; 
[m1,iter_stats1] = mcmc_levelset_pcn_eig_fill(beta,gamma,max_iter,V,E,fid); 
time_ = toc; 
disp(['Running MCMC took ', num2str(time_),  ' s']);

% Do the MCMC Probit Eig Experiment
disp('Starting MCMC Batch...'); 
tic; 
beta=0.5; % proposal variance/step
gamma=0.2; % obs noise std
max_iter=2*10^4; % number of mcmc steps
scale = max(S); 
E = S/scale; 
opt = {};  opt.rand_batch_sz = 500; 
[m2,iter_stats2] = mcmc_levelset_pcn_eig_fill(beta,gamma,max_iter,V,E,fid,opt); 
time_ = toc; 
disp(['Running MCMC took ', num2str(time_),  ' s']);
