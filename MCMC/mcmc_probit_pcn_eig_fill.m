function [ y_mean,iter_stats ] = mcmc_probit_pcn_eig_fill(beta,gamma,max_iter,V,E,fid,varargin)
%--------------------------------------------------------------------------
%  Description : 
%       Perform MCMC on Probit using eigenvectors + fill.
...
%                           Default True.  
%           fill_val: double, value to fill for rest of eivenvalues, 
%                     default max(E). 
...
for k=1:max_iter
    ...
    acceptance_prob(k) = min(1, exp(logpv - logpu)); 
    % accep-reject step
    r=rand;
