function [ y_mean,iter_stats ] = mcmc_gl_pcn_eig(beta,eta,eps, max_iter,V,E,fid,varargin)
%--------------------------------------------------------------------------
%  Description : 
...
for k=1:max_iter
    ...
    Phi_u = Phi(u, eps, eta, fid); 
    Phi_v = Phi(v, eps, eta, fid); 
...
function res = Phi(u, eps, eta, fid)
%res = sum((u.^2-1).^2)/eps;
res = 0; 
for i = 1:numel(fid{1})
    res = res + .5*eta*(u(fid{1}(i)) - 1)^2;
end
for i = 1:numel(fid{2})
    res = res + .5*eta*(u(fid{2}(i)) + 1)^2;
end
end
